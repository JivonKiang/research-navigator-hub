...
function verificationRecord(id) { return state.cloud.references?.[id] || {}; }
function refStatus(id) { return verificationRecord(id).status || 'unknown'; }
...
function renderMetrics() {
  ...
  const partial = refs.filter(r => refStatus(r.id) === 'partial').length;
...
function refCard(r) {
  ...
  const rec = verificationRecord(r.id);
  const d = doi(r);
  const checks = rec.checks || {};
  const checkLine = ['title','pmid','doi'].map(k => `${k.toUpperCase()}: ${checks[k] || 'missing'}`).join(' · ');
  const problems = (rec.problems || []).length ? `<div class="ref-note problem-note">问题：${(rec.problems || []).map(esc).join('；')}</div>` : '';
  const sources = (rec.sources || []).map((s, idx) => `<a href="${esc(s)}" target="_blank" rel="noopener">核验源 ${idx + 1}</a>`).join('');
  return `<article class="ref-card">
    <div class="ref-top">
      ...
      <span class="verify-btn ${st}" title="${esc(statusText(st))}">${mark(st)}</span>
    </div>
    <div class="ref-note">${esc(r.note || '')}</div>
    <div class="ref-note">自动核验：${esc(statusText(st))} · ${esc(checkLine)}${rec.verifiedAt ? ` · ${esc(rec.verifiedAt)}` : ''}</div>
    ${problems}
...
function renderReferences() {
  ...
  if (state.refFilter === 'unverified') refs = refs.filter(r => refStatus(r.id) !== 'verified');
  const allRefs = state.data.references;
  const verified = allRefs.filter(r => refStatus(r.id) === 'verified').length;
  const partial = allRefs.filter(r => refStatus(r.id) === 'partial').length;
  const problem = allRefs.filter(r => refStatus(r.id) === 'problem').length;
  const metadata = allRefs.filter(r => refStatus(r.id) === 'metadata').length;
  $('#verification-note').innerHTML = `自动核验：${verified} 条三重通过；${partial} 条 title+DOI 通过但缺 PMID；${problem} 条需处理；${metadata} 条为非论文元数据。`;
